...
async def get_profile(ticker: str) -> dict:
    try:
        ...
        _log.warning("FMP profile failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_key_metrics(ticker: str, limit: int = 5) -> list[dict]:
    try:
        ...
        _log.warning("FMP key-metrics failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_ratios(ticker: str, limit: int = 5) -> list[dict]:
    try:
        ...
        _log.warning("FMP ratios failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_income_statement(ticker: str, limit: int = 5, period: str = "annual") -> list[dict]:
    try:
        ...
        _log.warning("FMP income failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_balance_sheet(ticker: str, limit: int = 5, period: str = "annual") -> list[dict]:
    try:
        ...
        _log.warning("FMP balance failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_cash_flow(ticker: str, limit: int = 5, period: str = "annual") -> list[dict]:
    try:
        ...
        _log.warning("FMP cashflow failed for %s (%s) — falling back to yfinance", ticker, exc)
...
async def get_financial_growth(ticker: str, limit: int = 5) -> list[dict]:
    try:
        ...
        _log.warning("FMP growth failed for %s (%s) — falling back to yfinance", ticker, exc)
